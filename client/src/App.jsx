    ...
    <div className="container" onMouseMove={handleMouseMove}>
      <nav className="top-nav">
        <div className="logo" onClick={() => setShowCollection(false)}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="logo-icon">
            <rect x="2" y="6" width="14" height="12" rx="2" stroke="#22c55e" strokeWidth="2" />
            <path d="M16 16L22 19V5L16 8" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          CINESUGGEST
        </div>
      ...
      {!showCollection ? (
          ...
          <main>
            ...
            <div className="movie-grid">
              {movies.map((movie, index) => {
                ...
                return (
                  <article className="movie-card" key={index} style={{ animationDelay: `${index * 0.15}s` }}>
                    <div className="movie-content">
                      <header className="movie-header">
                        <h2>{movie.title}</h2>
                        <button
                          className={`save-btn ${isSaved ? 'saved' : ''}`}
                          onClick={() => toggleCollectionItem(movie)}
        ...
        <main className="collection-view">
          <header className="hero" style={{ marginTop: '4rem', marginBottom: '2rem' }}>
